...
interface LiveState {
  zones: Record<string, { densityLevel: Density }>;
}
...
export default function FanApp() {
  ...
  useEffect(() => {
    ...
    fetchAdvisory();
    const interval = setInterval(fetchAdvisory, 15000);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-wc-navy">
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          {advisory && (
            <div className="bg-wc-surface-hover border border-wc-cyan/30 rounded-xl p-4 flex items-start gap-3 shadow-lg shadow-wc-cyan/5">
              <Info className="text-wc-cyan shrink-0 mt-0.5" />
              <div>
                <h4 className="font-bold text-wc-text mb-1">Live Stadium Advisory</h4>
